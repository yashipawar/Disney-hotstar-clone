...
const MovieDetails = () => {
  ...
  return (
    ...
    backgroundRepeat:"no-repeat",backgroundSize:"1550px 950px"}} className='pl-10 pb-7 h-screen grid grid-cols-2 bg-black'>
      <div>
        <h1 className='text-slate-300 pt-60 font-bold text-4xl'>{location?.state?.data?.title ?? location?.state?.data?.name}</h1>
        <h1 className='mt-3 text-slate-300'>{location?.state?.data?.release_date ?? location?.state?.data?.first_air_date}</h1>
